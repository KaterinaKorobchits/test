...
class NavigatiionHelper(
    ...
    fun setupWithNavController(bottomNavigationView: BottomNavigationView) {
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener { item ->
            ...
            if (fragmentManager.isStateSaved) false else {
    ...
    fun onSupportNavigateUp(): Boolean {
        val navigateUp = selectedNavController.value?.navigateUp() ?: false
        return if (navigateUp) true else navigateFirst()
    }

    private fun navigateFirst(): Boolean {
        if(selectedItemTag == firstFragmentTag) return false

        fragmentManager.popBackStack(firstFragmentTag, POP_BACK_STACK_INCLUSIVE)
        val firstFragment = fragmentManager.findFragmentByTag(firstFragmentTag) as NavHostFragment
        selectedNavController.value = firstFragment.navController
        return true
    }
